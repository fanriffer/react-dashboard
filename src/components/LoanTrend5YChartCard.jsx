// src/components/LoanTrend5YChartCard.jsx
...
export default function LoanTrend5YChartCard({
  data,
  title = "แนวโน้มสินเชื่อ",
  subTitle = "สินเชื่อ (แนวโน้ม 5 ปี)",
  unitLeft = "สินเชื่อ",
  unitRight = "ปีบัญชี",
}) {
  ...
  const yMin = data?.length ? Math.min(...data.map((d) => d.accounts)) : 0
  const yMax = data?.length ? Math.max(...data.map((d) => d.accounts)) : 0
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      link.download = "loan-trend-5y.jpg"
      link.href = dataUrl
      link.click()
    } catch (err) {
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex-1 text-center">
          ...
          <div className={`text-sm mt-1 ${theme.sub}`}>{subTitle}</div>
        </div>
                ...
                <>
                  ...
                  <ReferenceLine
                    ...
                    label={{
                      ...
                      position: "top",
                      fill: isDark ? "rgba(255,255,255,0.90)" : "rgba(15,23,42,0.85)",
                      fontSize: 12,
                      fontWeight: 700,
                    }}
                  />
