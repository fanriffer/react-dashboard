...
import { memberMonthlyMock } from "../mock/memberMonthlyMock"
import { toJpeg } from "html-to-image"
import { useRef} from "react"


export default function MemberMonthlyChartCard() {
  const data = memberMonthlyMock

  // ตัวเลข summary มุมขวา (เอาแบบง่ายก่อน: สุทธิเดือนล่าสุด = in - out)
  const last = data[data.length - 1]
  const netLast = last.in - last.out // เช่น 94-78 = 16
  ...
  try {
    ...
    link.download = "members-monthly.jpg"
    link.href = dataUrl
    link.click()
  } catch (err) {
      ...
      <div className="flex flex-col gap-2 p-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <h3 className="mt-2 text-base font-semibold text-slate-900 dark:text-slate-100">
            สมาชิกเข้า–ออก รายเดือน (2568)
          </h3>
          <p className="text-xs text-slate-500 dark:text-slate-400">
            เปรียบเทียบจำนวนสมาชิกเข้าและออกในแต่ละเดือน
          </p>
        </div>

        {/* Summary */}
        <div className="flex items-center gap-2">
          <span className="text-xs text-slate-500 dark:text-slate-400">สุทธิเดือนล่าสุด</span>
      ...
      <div className="h-[220px] w-full px-2 pb-4 sm:h-[260px] sm:px-4">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 10, right: 10, left: -10, bottom: -10 }}>
            ...
            />
            <YAxis tick={{ fill:"#f6b902", fontSize: 15 }}
                   axisLine={{ stroke: "#f6b902" }} />

            <Tooltip />

            {/* เส้น "มุมแหลม" = type="linear" */}
            <Line
              type="linear"
              dataKey="in"
              strokeWidth={2}
